...
# ---------------------------------------------------------
# ✅ PATH SETUP (DATASET IS NOW INSIDE ml_model)
# ---------------------------------------------------------
# 🔁 CHANGE 1: BASE_DIR now correctly points to ml_model/
BASE_DIR = Path(__file__).resolve().parent   # ✅ ml_model/

# 🔁 CHANGE 2: DATA_DIR now directly points to ml_model/datasets/
DATA_DIR = BASE_DIR / "datasets"             # ✅ ml_model/datasets/

# 🔁 CHANGE 3: MODEL_DIR stays as ml_model/ because .pkl files are here
MODEL_DIR = BASE_DIR                         # ✅ ml_model/
 
# ---------------------------------------------------------
# ✅ LOAD ALL DATASETS (FROM ml_model/datasets)
# ---------------------------------------------------------
...
for veg, path in DATASETS.items():
    ...
    # 🔁 CHANGE 4: SAFETY CHECK so the app fails fast if a file is missing
    if not path.exists():
...
# ---------------------------------------------------------
# ✅ LOAD MODELS (FROM ml_model/)
# ---------------------------------------------------------

def load_model(veg):
    file_path = MODEL_DIR / f"{veg}_model.pkl"   # 🔁 CHANGE 5: Pathlib style
    if file_path.exists():
        return joblib.load(file_path)
    return None
...
def predict_price():
    ...
    # 🔁 CHANGE 6: Scaler also loads directly from ml_model/
    scaler_path = MODEL_DIR / "temp2.pkl"
